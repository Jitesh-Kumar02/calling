...
import { callMap, callInviteMap, voice } from "../../../util/voice";
...
>(
  ...
  async ({ id }, { dispatch, getState, rejectWithValue }) => {
    ...
    //console.log("this is the accept result", acceptResult, callInvite);
    // const acceptResult = voice.connect(callInvite);
    console.log("call parameters", {
      To: `+${callInvite.getTo()}`,
      uuId: callInvite.getCallSid(),
      ...
      accept: "true",
      callNotFromCampaign: "true",
      Web: "false",
    });
    const acceptResult = await settlePromise(
...
>(
  ...
  async ({ id }, { dispatch, getState, rejectWithValue }) => {
    ...
    const rejectCallInstance = await auth.rejectCall(
      userData.userId,
      callInvite.getCallSid()
    );
    const rejectResult = await settlePromise(rejectCallInstance.reject());
